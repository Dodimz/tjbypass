...
class PaymentReportController extends Controller
{
    ...
    public function verify(Request $request, int $id)
    {
        ...
        } elseif ($payment->purchase_type === ServiceOrder::class) {
            $this->markServiceOrderPaid($payment);
        }

        $notificationData = [
            'title' => 'Offline payment verified for '.$this->getPurchaseTypeLabel($payment->purchase_type),
            'url' => $payment->purchase_type === ServiceOrder::class
    ...
    private function getPurchaseTypeLabel(string $purchaseType): string
    {
        return match ($purchaseType) {
            Exam::class => 'Exam',
            ServiceOrder::class => 'Service Order',
            default => 'Course',
        };
    }

    /**
     * Mark a pending service order as paid after offline verification.
     */
    private function markServiceOrderPaid(PaymentHistory $payment): void
    {
        /** @var ServiceOrder|null $order */
        $order = ServiceOrder::query()->with('service')->find($payment->purchase_id);

        if ($order && $order->status === ServiceOrderStatus::PENDING) {
            $order->update([
            ...
            if (! empty($order->technician_id)) {
                User::find($order->technician_id)?->notify(new ServiceOrderNotification([
