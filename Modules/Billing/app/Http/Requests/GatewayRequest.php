...
class GatewayRequest extends FormRequest
{
     ...
     *
     * Inertia forms submit booleans as strings ("true"/"false") via FormData,
     * which Laravel's boolean rule rejects. Normalize them before validating.
     */
    public function prepareForValidation(): void
    {
        $this->merge(collect([
            'active' => $this->input('active'),
            'test_mode' => $this->input('test_mode'),
        ])
    ...
    public function rules(): array
    {
        ...
        $rules = array_merge($rules, $this->getGatewaySpecificRules());

        // Allow saving a disabled gateway without complete credentials.
        // Credentials are only mandatory when the gateway is being enabled.
        if (! request()->boolean('active')) {
            foreach ($rules as $field => $rule) {
                if (in_array($field, ['type', 'active'], true)) {
                    continue;
                }
    ...
    /**
     * Get validation rules for Midtrans
     */
    private function midtransRules(): array
    {
        $rules = [
            'test_server_key' => 'nullable|string|max:255',
            'test_client_key' => 'nullable|string|max:255',
            'live_server_key' => 'nullable|string|max:255',
            'live_client_key' => 'nullable|string|max:255',
        ];

        // Make test keys required when test_mode is true
        if (isset($this->test_mode) && request('test_mode')) {
            $rules['test_server_key'] = 'required|string|max:255';
            $rules['test_client_key'] = 'required|string|max:255';
        }

        // Make live keys required when test_mode is false
        else {
            $rules['live_server_key'] = 'required|string|max:255';
            $rules['live_client_key'] = 'required|string|max:255';
        }
