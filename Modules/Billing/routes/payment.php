...
use Modules\Billing\Http\Controllers\Payment\MidtransController;
...
use Modules\Billing\Http\Controllers\Payment\WalletController;
use Modules\Billing\Http\Controllers\Payment\WalletReportController;
...
Route::middleware(['auth', 'role:admin'])->prefix('dashboard')->group(function () {
    ...
    // wallet deposit verification routes
    Route::controller(WalletReportController::class)->prefix('billings/wallet-deposits')->group(function () {
        Route::get('/', 'index')->name('wallet-reports.index');
        Route::post('{id}/verify', 'verify')->name('wallet-reports.verify');
        Route::post('{id}/reject', 'reject')->name('wallet-reports.reject');
    });
});
...
Route::middleware(['auth', 'verified'])->prefix('payments')->group(function () {
    ...
    // pay checkout item with wallet balance
    Route::post('wallet/payment', [WalletController::class, 'pay'])->name('payments.wallet.payment');
    ...
    // Midtrans
    Route::post('midtrans/payment', [MidtransController::class, 'payment'])->name('payments.midtrans.payment');
    Route::get('midtrans/success', [MidtransController::class, 'success'])->name('payments.midtrans.success');
    Route::get('midtrans/cancel', [MidtransController::class, 'cancel'])->name('payments.midtrans.cancel');
...
// Midtrans webhook notification (CSRF exempt, no auth required)
Route::withoutMiddleware([PreventRequestForgery::class])->post('payments/midtrans/notification', [MidtransController::class, 'notification'])->name('payments.midtrans.notification');
