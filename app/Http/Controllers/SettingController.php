...
class SettingController extends Controller
{
    ...
    /**
     * Display the dashboard menus visibility settings.
     */
    public function menus(Request $request)
    {
        $menus = $this->settingsService->firstOrCreateMenusSetting();

        return Inertia::render('dashboard/settings/menus', compact('menus'));
    }

    /**
     * Update the dashboard menus visibility settings.
     */
    public function menus_update(Request $request, string $id)
    {
        $validated = $request->validate([
            'job-circulars' => ['required', 'boolean'],
            'newsletters' => ['required', 'boolean'],
            'certification' => ['required', 'boolean'],
            'courses' => ['required', 'boolean'],
            'exams' => ['required', 'boolean'],
        ]);

        $this->settingsService->menusUpdate($validated, $id);
