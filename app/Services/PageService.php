...
class PageService extends PageSectionService
{
    ...
    {
        return Page::withRelatedData()->find($id);
    }

    public function getPageBySlug(string $slug)
    {
        return Page::bySlug($slug)->with(['sections' => function ($query) {
    ...
    {
        return Page::bySlug($slug)->withRelatedData()->first();
    }

    public function getActiveInnerPages()
    {
        return Page::where('type', 'inner_page')
            ->where('active', true)
            ->get();
    }
    ...
    public function getPageSections(array $data, Page $page)
    {
        ...
        $sections['courses'] = $this->course->getCourses([
            ...$data,
            'status' => 'approved',
            'select' => 'id,title',
            'mode' => 'public',
        ...
        $sections['services'] = $this->getServices($data);
        ...
        $sections['topServices'] = $this->getTopServices($idCollection['top_services']);
    ...
    public function getTopServices(array $serviceIds)
    {
    ...
    public function getServices(array $data): \Illuminate\Contracts\Pagination\LengthAwarePaginator
    {
        $perPage = array_key_exists('service_per_page', $data) ? intval($data['service_per_page']) : 10;

        return Service::query()
            ->when(array_key_exists('service', $data), function ($query) use ($data) {
                return $query->where('title', 'LIKE', '%'.$data['service'].'%');
            })
            ->published()
