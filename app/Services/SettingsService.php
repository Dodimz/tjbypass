...
{
    /**
     * Dashboard menu groups that can be hidden by the admin.
     *
     * @var list<string>
     */
    public const MENU_KEYS = ['job-circulars', 'newsletters', 'certification', 'courses', 'exams'];
    ...
    /**
     * Get or create the dashboard menus visibility setting.
     */
    public function firstOrCreateMenusSetting(): Setting
    {
        return DB::transaction(function () {
            return Setting::firstOrCreate(
                ...
                [
                    ...
                    'fields' => array_fill_keys(self::MENU_KEYS, true),
                ]
            );
        }, 5);
    }
    ...
    /**
     * Visibility map for every hideable dashboard menu group.
     *
     * @return array<string, bool>
     */
    public function getMenuVisibility(): array
    {
        $setting = $this->getSetting(['type' => 'menus']);

        $visibility = [];

        foreach (self::MENU_KEYS as $key) {
            $visibility[$key] = (bool) ($setting->fields[$key] ?? true);
        }
    ...
    /**
     * Slugs of dashboard menu groups that are currently hidden.
     *
     * @return list<string>
     */
    public function getHiddenMenus(): array
    {
        return array_keys(array_filter(
            ...
            fn (bool $visible): bool => ! $visible
        ));
    }

    public function menusUpdate(array $data, string $id): Setting
    {
        return DB::transaction(function () use ($data, $id) {
            return tap(Setting::findOrFail($id), fn (Setting $setting) => $setting->update(['fields' => $data]));
        }, 5);
    }
